...
orders = spark.read.format("delta").table("vtex_db.total_df_final")
...
# Create a DataFrame from the Delta table registered in the Hive metastore
items = spark.read.format("delta").table("vtex_db.items_df_final")
...
# Filter the DataFrame to include only completed orders
completed_orders_count = orders1.filter(col("name") == "completed").count()
...
cancelled_orders_count = orders2.filter(col("value") == "canceled").count()
...
completed_orders_count = orders5.filter(col("completed_orders") == "true").count()
...
canceled_orders_count = orders6.filter(col("cancelled_orders") == "canceled").count()
...
# Create a DataFrame from the Delta table registered in the Hive metastore
client_profile = spark.read.format("delta").table("vtex_db.client_profile_data_df_final")
...
# MAGIC %run /Users/deepa.r2019@vitstudent.ac.in/project_implementation/bronze_to_silver/Common_functions
